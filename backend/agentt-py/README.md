...
`app/` 按 Agent 四大能力（规划 / 记忆 / 工具 / 行动）组织，详见 `ARCHITECTURE.md`：

```
├── main.py            # FastAPI 入口：SSE 接口 + 静态页
├── static/index.html  # 极简查看页
├── smoke_test.py      # 无 key 冒烟测试（假 LLM 验证图结构）：python smoke_test.py
├── ARCHITECTURE.md    # 四大件结构与开发约定
└── app/
    ├── config.py      # 读 .env（公共服务）
    ├── graph.py       # LangGraph 组装（公共服务）
    ├── planning/      # 规划
    │   ├── supervisor.py  # 主管节点 + 路由
    │   ├── planner.py     # SimplePlanner 任务规划器
    │   └── demo.py        # 规划演示
    ├── memory/        # 记忆
    │   └── state.py       # AgentState 共享状态
    ├── tools/         # 工具
    │   └── image.py       # 文生图（CogView）
    └── action/        # 行动
...
- **任务规划**：`app/planning/` 提供不依赖 LLM 的 `SimplePlanner`（`Plan` / `PlanStep`），演示 `python -m app.planning.demo`；后续接模型只替换计划生成逻辑。
- **换生图后端**：改 `app/tools/image.py` 的 `generate_image()` 即可（签名 `(prompt) -> url` 不变），SDXL / 3DGS 接进同一个函数。
- **防死循环**：主管单次请求最多调度 12 轮（`app/planning/supervisor.py: MAX_STEPS`）。
